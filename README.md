...
| :--- | :--- |
| **include/stb_image.h** | Biblioteca externa responsável pela decodificação de imagens (PNG/JPG). É a base para o carregamento de qualquer asset visual do projeto. |
...
| **src/render_utils.c <br> include/render_utils.h** | Centraliza funções auxiliares de renderização: configuração de materiais, ativação/limpeza de emissão própria, desenho de esferas texturizadas e órbitas. |
...
| **src/station.c <br> include/station.h** | Gerencia a Estação Espacial Internacional: carrega o modelo via `obj_loader`, posiciona em órbita baixa ao redor da Terra e realiza sua rotação e translação. |
